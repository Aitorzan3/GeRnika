...
get_children<-function(phylotree, node_idx){
  ...
  return(unlist(map(children_clones, function(x) clone_to_gene(phylotree,x))))
}


get_descendants <- function(phylotree, node_idx) {
  children<-get_children_idx(phylotree@B, gene_to_clone(phylotree,node_idx))
  children_mutations<-c(map(children, function(x) clone_to_gene(phylotree,x)))
  if (length(children)) {
    return(unlist(c(children_mutations, map(children_mutations, function(x) get_descendants(phylotree, x)))))
  } else {
    return()
  }
}
