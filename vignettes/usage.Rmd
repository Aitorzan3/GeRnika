...
| `noise`            | Add sequencing noise to the values in the $\boldsymbol{F}$ matrix. | Boolean (`TRUE` by default) |
...
```{r, message = FALSE, eval = FALSE,  fig.dim = c(6.5,6)}
...
```{r, echo = FALSE, message = FALSE, warning = FALSE, fig.align ="center", fig.dim = c(6.5,6), fig.cap="On the left the plot of `tree1` (`k=0`), on the right the plot of `tree2` (`k=8`)." }
# Simulate a tumor with k=0:
...
U_to_heatmap <- function(U, values = TRUE, col_names = c("samples", "clones", "proportion")){
  ...
  Var1 <- col_names[1]
  Upos<-ggplot(Upos, aes(x=samples, y=clones, fill=proportion)) + geom_tile(col="black") +# theme_bw()    +    
    theme(
...
```{r, message = FALSE, eval = FALSE}
# Function to plot heatmap of the U matrix
U_to_heatmap <- function(U, values = TRUE, col_names = c("samples", "clones", "proportion")) {
  Upos <- melt(U)
  colnames(Upos) <- col_names
  Var1 <- col_names[1]
...
```{r, message = FALSE, echo = FALSE,  warning = FALSE, fig.align ="center", fig.dim = c(7.5,8), fig.cap="Heatmaps of the $\boldsymbol{U}$ matrices of an instance of a tumor under positive selection (top) and neutral evolution (bottom)."}
# simulate a tumor with neutral selection:
Ipos <- create_instance(n=5, m=8, k=0.5, selection="positive", seed = 1)

# simulate a tumor with positive selection:
Ineu <- create_instance(5,8,0.5,"neutral", seed = 1)
...
```{r, message = FALSE, echo = FALSE, warning = FALSE, fig.align ="center", fig.dim = c(7.5, 4), fig.cap="The effect of noise."}
# Simulate a tumor with sequencing noise added:
Inoisy <- create_instance(5,8,0.5,"neutral",noisy = TRUE,depth = 5, seed = 1)
...
```{r, echo = FALSE, out.width="100%", caption = "The evolution of the error in VAF values for a tumor composed by 10 clones and 2 samples."}
...
```{r, echo = FALSE, out.width="100%", caption = "The evolution of the error in VAF values for a tumor composed by 100 clones and 10 samples."}
...
| clones                | Clones in the $\boldsymbol{B}$ matrix of the tumor                                          |  Vector |
| genes                | Genes that have mutated according to the $\boldsymbol{B}$ matrix of the tumor                                |  Vector      |
...
```{r, echo = FALSE}
# Simulate a tumor with 5 clones, 4 samples, k=0.5:
instance <- create_instance(n=5, m=4, k=0.5, selection="positive", noisy=FALSE, seed = 1)
...
This is one of the possible methods for instantiating `Phylotree` class objects on the basis of the $\boldsymbol{B}$ matrix of a tumor. However, this package also grants the option of using the general constructor for the `Phylotree` S4 class for instantiating new `Phylotree` objects, which allows users to give custom values to the attributes of a new `Phylotree` class object.
...
```{r, eval=FALSE, fig.dim = c(7.5,5), fig.cap="Phylogenetic tree of 5 clones using proportions. In this case, there are 4 plots because we have generated an instance based on 4 samples. Then, each tree represents the proportions of the clones in each sample."}
# Simulate a tumor
...
```{r, echo=FALSE, fig.dim = c(7.5,5), fig.cap="Phylogenetic tree of 5 clones using proportions. In this case, there are 4 plots because we have generated an instance based on 4 samples. Then, each tree represents the proportions of the clones in each sample."}
# Simulate a tumor
instance <- create_instance(5, 4, 0.5, selection="neutral", noisy=FALSE, seed=1)

#Use the B matrix of the previously generated tumor for creating a Phylotree class object
tree<-B_to_phylotree(instance$B)

#Plot the phylogenetic tree using the U matrix of the previously generated tumor
plot_proportions(tree, instance$U)
...
```{r fig.align ="center", out.width="-1.5in",eval = FALSE, message = FALSE, warning = FALSE}
